...
void D3D12ExecuteIndirect::LoadPipeline()
{
    ...
    // Create frame resources.
    {
        ...
        for (UINT n = 0; n < FrameCount; n++)
        {
            ...
            ThrowIfFailed(m_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_computeCommandAllocators[n])));
        }
    }
}
...
void D3D12ExecuteIndirect::LoadAssets()
{
    // Create the root signatures.
    {
        ...
        computeRootSignatureDesc.Init_1_1(_countof(computeRootParameters), computeRootParameters, 0, nullptr, D3D12_ROOT_SIGNATURE_FLAG_CBV_SRV_UAV_HEAP_DIRECTLY_INDEXED);
    ...
    // Create the pipeline state, which includes compiling and loading shaders.
    {
        ...
        CompileShader(L"compute.hlsl", L"CSMain", L"cs_6_6", {}, computeShader);
    ...
    ThrowIfFailed(m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_computeCommandAllocators[m_frameIndex].Get(), m_computeState.Get(), IID_PPV_ARGS(&m_computeCommandList)));
...
void D3D12ExecuteIndirect::PopulateCommandLists()
{
    ...
    if (m_enableCulling)
    {
        ...
        //    SrvUavTable,
        //    CD3DX12_GPU_DESCRIPTOR_HANDLE(cbvSrvUavHandle, CbvSrvOffset + 0, m_cbvSrvUavDescriptorSize));

        m_computeCommandList->SetComputeRoot32BitConstants(RootConstants, 4, reinterpret_cast<void*>(&m_csRootConstants), 0);

        // Reset the UAV counter for this frame.
        m_computeCommandList->CopyBufferRegion(m_processedCommandBuffers[0].Get(), CommandBufferCounterOffset, m_processedCommandBufferCounterReset.Get(), 0, sizeof(UINT));

        D3D12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(m_processedCommandBuffers[0].Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    ...
    // Record the rendering commands.
    {
        ...
        D3D12_RESOURCE_BARRIER barriers[2] = {
            CD3DX12_RESOURCE_BARRIER::Transition(
                m_enableCulling ? m_processedCommandBuffers[0].Get() : m_commandBuffer.Get(),
        ...
        if (m_enableCulling)
        {
            ...
            m_commandList->ExecuteIndirect(
                ...
                TriangleCount,
                m_processedCommandBuffers[0].Get(),
                0,
                m_processedCommandBuffers[0].Get(),
...
void D3D12ExecuteIndirect::CompileShader(const std::wstring& filename, const std::wstring& entryPoint, const std::wstring& target, std::vector<DxcDefine> defines, Microsoft::WRL::ComPtr<ID3DBlob>& shaderBlob) {
    ...
    if (FAILED(hr)) {
        std::wcout << ("Failed to create include handler.") << std::endl;
        ThrowIfFailed(hr);
        return;
    }
    ...
    std::wcout << "Compiling with arguments: " << std::endl;
    ...
    if (FAILED(hr)) {
        ...
        if (pErrors && pErrors->GetStringLength() > 0) {
            std::wcerr << ("Shader compilation failed: {}", pErrors->GetStringPointer()) << std::endl;
        }
        ThrowIfFailed(hr);
        return;
    }
    ...
    result->GetOutput(DXC_OUT_ERRORS, IID_PPV_ARGS(pErrors.GetAddressOf()), nullptr);
    if (pErrors && pErrors->GetStringLength() > 0) {
        std::wcerr <<("Shader compilation warnings/errors: {}", pErrors->GetStringPointer()) << std::endl;
